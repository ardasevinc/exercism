...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    """
    temp_times_n_emitted = temperature * neutrons_emitted
    if temperature < 800 and neutrons_emitted > 500 and temp_times_n_emitted < 500_000:
        return True
    else:
        return False
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    threshold_10p = threshold * 0.1
    threshold_plus10p = threshold + threshold_10p
    threshold_minus10p = threshold - threshold_10p
    
    if temp_neutron_product < 0.9 * threshold:
        return "LOW"
    elif (
        temp_neutron_product < threshold_minus10p
        or temp_neutron_product < threshold_plus10p
    ):
        return "NORMAL"
    else:
        return "DANGER"
